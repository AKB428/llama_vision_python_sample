...
def process_file(inputImageFile):
    try:
        ...
        if response_details and parsed_content:
            ...
            return parsed_content.get('C1', 0), parsed_content.get('C2', 0)
    except Exception as e:
        print(f"Error processing file {inputImageFile}: {e}")
        return 'error', 'error'

# 再帰的にフォルダ内のファイルを処理
def process_directory(directory, move_files=False):
    general_folder = os.path.join(os.path.dirname(directory), f"{os.path.basename(directory)}-general")
    r18_folder = os.path.join(os.path.dirname(directory), f"{os.path.basename(directory)}-r18")

    if move_files:
        if not os.path.exists(general_folder):
            os.makedirs(general_folder)
        if not os.path.exists(r18_folder):
            os.makedirs(r18_folder)
    ...
    for root, _, files in os.walk(directory):
        for file in files:
            ...
            if file_path.lower().endswith(('.jpg', '.jpeg', '.png')):  # 対応する画像拡張子
                try:
                    c1, c2 = process_file(file_path)
                    if move_files and c1 != 'error' and c2 != 'error':
                        if c1 == 0 and c2 == 0:
                            shutil.move(file_path, os.path.join(general_folder, file))
                        elif c1 == 1 or c2 == 1:
                            shutil.move(file_path, os.path.join(r18_folder, file))
                except Exception as e:
...
if __name__ == "__main__":
    ...
    move_files = '-m' in sys.argv

    if os.path.isfile(input_path):
        process_file(input_path)
    elif os.path.isdir(input_path):
        process_directory(input_path, move_files)
    else:
